...
unsigned int NetTimingBufferL2::SortedPos (int difference) {
	...
	unsigned int top = m_bufferFull ? NET_L2TIMINGBUFFERSIZE : m_receivedPos;

	while (top > bottom) {
		unsigned int mid = (top + bottom) >> 1;
...
void NetTimingBufferL2::LogDifference (int difference) {
	...
	if (insertPos < removePos) {
		memmove (
			&m_sortedIndex [insertPos + 1], 
			&m_sortedIndex [insertPos], 
			(removePos - insertPos) * sizeof (m_sortedIndex [0]));
	}
	else if (insertPos > removePos) {
        insertPos--;
        if (insertPos > removePos)
            memmove (
                &m_sortedIndex [removePos],
                &m_sortedIndex [removePos + 1],
                (insertPos - removePos) * sizeof (m_sortedIndex [0]));
	}
